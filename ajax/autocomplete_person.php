...
/*  ajax/autocomplete_person.php
    ...
    >>>00004: no check for anything about Customer, so this will find people with no relation to current customer.
...
if (!$v->validate()) {
    $logger->error2('637267950426086460', "Error input parameters ".json_encode($v->errors()));
	header('Content-Type: application/json');
    echo $v->getErrorJson();
    die();
}
...
if (!$result) {
    $logger->errorDb('637267950327261564', 'Select person details failed: Hard DB error', $db);
}
