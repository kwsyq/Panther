...
class task_jobById extends API {
	...
	/*
    INPUT $_REQUEST['jobId']: Primary key into DB table Jobs
    ...
    Sets data as two key-value pairs (that is, an associative array with two elements):
    
    * 'job': an array that should have a single element, which is itself an associative array with the following members:
        * 'jobId': jobId
        * 'number': Job Number, e.g. 's1906007'
	...
	public function run() {
		...
		if ($row) {			
			...
			if (intval($job->getJobId())) {
				...
				$jobArray = array();
				$jobArray['jobId'] = $job->getJobId();
				$jobArray['number'] = $job->getNumber();				
				
				$this->setStatus('success');
				$this->setData(array('key' => 'workOrders', 'value' => $ret));				
				$this->setData(array('key' => 'job', 'value' => $jobArray));
			}
		}
