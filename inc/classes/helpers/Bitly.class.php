<?php
/* Bitly.class.php
   EXECUTIVE SUMMARY: Simple PHP library for interacting with the v3 bit.ly api,
   which makes "short" URLs. As of 2019-03, we are probably not using this anywhere.
    
   It looks like this is pretty much straight third-party code except for introducing
   PHP const to replace PHP define, which should be harmless.
   
   * Public methods:
   ** bitly_oauth_access_token($code, $redirect, $client_id, $client_secret)
   ** bitly_oauth_access_token_via_password($username, $password, $client_id, $client_secret)
   ** bitly_get($endpoint, $params, $complex=false)
   ** bitly_post($api_endpoint, $params)
   ** bitly_get_curl($uri)
   ** bitly_post_curl($uri, $fields, $header_array = array())
   
   No constructor, no local data; all of this could have been implemented as static, but it's OK as it is.
...
//	define('bitly_oauth_access_token', 'https://api-ssl.bit.ly/oauth/');
	 ...
	 */
	function bitly_post($api_endpoint, $params) {
	  $result = array();
