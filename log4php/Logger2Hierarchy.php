...
class Logger2Hierarchy {
	 ...
	 */
	public function __construct(Logger2Root $root) {
		$this->root = $root;
	...
	public function getLogger($name) {
		if(!isset($this->loggers[$name])) {
			...
			if($firstNode != $name and isset($this->loggers[$firstNode])) {
				$logger->setParent2($this->loggers[$firstNode]);
			} else {
				// if there is no father, set root logger as father
				$logger->setParent2($this->root);
			} 
			...
			if(count($nodes) > 0) {
				...
				foreach($nodes as $node) {
					...
					if(isset($this->loggers[$parentNode]) and $parentNode != $name) {
						$logger->setParent2($this->loggers[$parentNode]);
					}
					$firstNode .= ".$node";
				}
			}
