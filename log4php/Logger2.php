<?php
/* log4php/Logger2.php
   The Logger2 class extends the Logger class. For 6 Logger methods, it adds analogous methods which allow us to 
   implement the unique ID of each line of code that writes to the log. The idea here is that this code will remain
   as an ID even when line numbers inevitably change.

The new public methods are:
...
These methods each have an extra parameter, $id, which is prepended to the log message, with a hyphen in between. 
The "Db" methods take a DB object as an additional parameter, and will report the last query & the last error or warning 
 for the relevant database handle.  
...
class Logger2 extends Logger {
	...
	public function setParent2(Logger2 $logger) {
		$this->parent = $logger;
	} 
	 ...
	 */
	public function trace2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelTrace(), $id." - ".$message, $throwable);
	} 		
	 ...
	 *   in the logging event.
	 */
	public function debug2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelDebug(), $id." - ".$message, $throwable);
	} 
	 ...
	 *   in the logging event.
	 */
	public function info2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelInfo(), $id." - ".$message, $throwable);
	}
	 ...
	 *   in the logging event.
	 */
	public function warn2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelWarn(), $id." - ".$message, $throwable);
	}
	 ...
	 *   in the logging event.
	 */
	public function error2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelError(), $id." - ".$message, $throwable);
	}
	...
	* @param mixed $message message
	* @param DB $database object used to log errors
	* @param Exception $throwable Optional throwable information to include 
	*
	*/	
	public function warnDb($id, $message, $database, $throwable = null) {

		$dbMess=$database->error;

		if($message==""){

			$this->log(LoggerLevel::getLevelWarn(), $id." => DbError [".$dbMess."] Query: [".$database->getQuery()."]" , $throwable);

		} else {

			$errMess=print_r($message, true);

			$this->log(LoggerLevel::getLevelWarn(), $id." - ".$errMess." => DbError [".$dbMess."] Query: [".$database->getQuery()."]" , $throwable);
	...
	*
	*/	
	public function errorDb($id, $message, $database, $throwable = null) {

		$dbMess=$database->error;

		if($message==""){

			$this->log(LoggerLevel::getLevelError(), $id." => DbError [".$dbMess."] Query:[".$database->getQuery()."]" , $throwable);
		} else {

			$errMess=print_r($message, true);

			$this->log(LoggerLevel::getLevelError(), $id." - ".$errMess." => DbError [".$dbMess."] Query: [".$database->getQuery()."]" , $throwable);
	 ...
	 *   in the logging event.
	 */
	public function fatal2($id, $message, $throwable = null) {
		$this->log(LoggerLevel::getLevelFatal(), $id." - ".$message, $throwable);
	}
	...
	public function getEffectiveLevel() {
		for($logger = $this; $logger !== null; $logger = $logger->getParent2()) {
	...
	public static function getHierarchy() {
		if(!isset(self::$hierarchy)) {
			self::$hierarchy = new Logger2Hierarchy(new Logger2Root());
		}
	...
	private static function getConfigurator($configurator = null) {
		if ($configurator === null) {
			return new Logger2ConfiguratorDefault();
		}
		
		if (is_object($configurator)) {
			if ($configurator instanceof Logger2Configurator) {
				return $configurator;
			} else {
