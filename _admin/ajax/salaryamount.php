...
/* _admin/ajax/rateamount.php
    ...
    EXECUTIVE SUMMARY: Update salaryAmount for indicated row in DB table customerPersonPayPeriodInfo as specified.
    ...
    INPUT: $_REQUEST['value']. New salaryAmount for the indicated row in DB table customerPersonPayPeriodInfo. Should be unsigned INT.
		...
		if (intval($parts[0])) {
		    if (intval($parts[1])) {
				if (is_numeric($value)) {
					...
					$query .= " salaryAmount = " . intval($value) . " ";
